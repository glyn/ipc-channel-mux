...
impl SelectableMultiReceiver {
    ...
    fn handle(mr: Arc<SelectableMultiReceiver>, msg: MultiMessage) -> Result<(), MuxError> {
        ...
        if let MultiMessage::Data(scid, payload, ipc_senders, shmems, ipc_channel_senders, ipc_channel_receivers) = msg {
            demuxer.send(scid, payload, &ipc_senders, mr.ipc_receiver_uuid, shmems, ipc_channel_senders, ipc_channel_receivers)
        } else {
